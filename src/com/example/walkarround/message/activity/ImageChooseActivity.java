...
import com.example.walkarround.message.adapter.ImageListAdapter;
import com.example.walkarround.message.adapter.ImageListAdapter.ImageItemListener;
import com.example.walkarround.message.util.MessageUtil;
import com.example.walkarround.util.Logger;
import com.example.walkarround.util.image.DirectoryFilterAdapter;
 ...
 * @author cmcc
 */
public class ImageChooseActivity extends Activity implements OnClickListener, ImageItemListener, OnItemClickListener {
    ...
    private void initBaseView() {
        ...
        case FROM_MESSAGE_CODE:
            ...
            mPreviewBtn.setText(R.string.msg_select_pics_preview);
    ...
    protected void sendImages(ArrayList<String> sendingList) {
        if (sendingList == null || sendingList.size() == 0) {
            Toast.makeText(ImageChooseActivity.this, R.string.msg_select_at_least_1_pic, Toast.LENGTH_SHORT).show();
        } else {
    ...
    private List<Map<String, String>> initAdapterList() {
        ...
        for (Map.Entry<String, ArrayList<String>> entry : directorys.entrySet()) {
            if (entry.getValue().size() > 0) {
                ...
                tempMap.put(
                        "count",
                        getResources().getString(R.string.msg_pic_total_count,
    ...
    public void onChoseCountChange(int choseCount) {
        ...
        if (choseCount == 0) {
            ...
            mPreviewBtn.setText(R.string.msg_select_pics_preview);
            mChoseOkBtn.setText(R.string.common_select);
        } else {
            mPreviewBtn.setEnabled(true);
            mPreviewBtn.setText(String.format(getResources().getString(R.string.msg_select_pics_preview_multi),
                    choseCount));
            if (!mIsFullSizeOption) {
                mChoseOkBtn.setText(String
                        .format(getResources().getString(R.string.msg_select_pics_finish_multi), choseCount, mMaxNum));
            } else {
                mChoseOkBtn.setText(String
                        .format(getResources().getString(R.string.msg_select_pics_send_multi), choseCount, mMaxNum));
            }
        }
    }
    ...
    public void onCameraClick() {
        // 打开照相机进行拍照
        mCameraImagePath = MessageUtil.createCameraTakePicFile();
        if (TextUtils.isEmpty(mCameraImagePath)) {
    ...
    public class GetImagePathTask extends AsyncTask<Boolean, ArrayList<String>, ArrayList<String>> {
        ...
        public ArrayList<String> getImagePathList(PhotoResource resource) {
            ...
            switch (resource) {
            case ALL:
                Colums = new String[] { Media.DATA, Media._ID, Media.TITLE, Media.DISPLAY_NAME, Media.DATE_ADDED };
                cursor = getContentResolver().query(Media.EXTERNAL_CONTENT_URI, Colums, null, null,
                        Media.DATE_ADDED + " DESC");
                if (cursor != null) {
                    while (cursor.moveToNext()) {
                        String tempString = cursor.getString(cursor.getColumnIndex(Media.DATA));
            ...
            case ONLY_CAMERA:
                ...
                Colums = new String[] { Media.DATA, Media._ID, Media.TITLE, Media.DISPLAY_NAME, Media.DATE_ADDED };
                cursor = getContentResolver().query(Media.EXTERNAL_CONTENT_URI, Colums,
                        Media.DATA + " like '%DCIM%'" + " OR " + Media.DATA + " like '%Camera%' ", null,
                        Media.DATE_ADDED);
                if (cursor != null) {
                    cursor.moveToLast();
                    do {
