...
/**
 * PinyinUnit as a base data structure to save the string that Chinese characters converted to Pinyin characters. for
 * example: Reference: http://www.cnblogs.com/bomo/archive/2012/12/25/2833081.html Chinese characters:"Hi你说了什么git???"
 * Pinyin: Hi-Hi ===>mPinyin=false, mPinyinBaseUnitIndex.size=1, mStartPosition=0
...
public class PinyinInfo implements Serializable {
     ...
     * 
     */
    private static final long serialVersionUID = -3399084362636514633L;
    // Whether Pinyin
    private boolean mPinyin;
    private int mStartPosition; // save starting index position that the variables in the original string.
    /*
     * save the string which single Chinese characters Pinyin(include Multiple Pinyin),or continuous non-kanji
     * characters. if mPinyinBaseUnitIndex.size not more than 1, it means the is not Polyphonic characters.
     */
    private List<PinyinBaseInfo> mPinyinBaseUnitIndex;

    public PinyinInfo() {
        mPinyin = false;
        mStartPosition = -1;
        mPinyinBaseUnitIndex = new ArrayList<PinyinBaseInfo>();
    }
    ...
    public List<PinyinBaseInfo> getPinyinBaseUnitIndex() {
        return mPinyinBaseUnitIndex;
    }

    public void setStringIndex(List<PinyinBaseInfo> stringIndex) {
