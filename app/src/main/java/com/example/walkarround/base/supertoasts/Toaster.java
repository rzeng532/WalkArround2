...
class Toaster extends Handler {
    ...
    private class SuperToastComparator implements Comparator<SuperToast> {
        ...
        public int compare(SuperToast x, SuperToast y) {
            ...
            if (x.getStyle().priorityLevel < y.getStyle().priorityLevel){
                return -1;
            }else if (x.getStyle().priorityLevel > y.getStyle().priorityLevel){
                return 1;
            } else {// PriorityQueue uses a heap, we want to maintain insertion order
                return x.getStyle().timestamp <= y.getStyle().timestamp ? -1 : 1;
            }
        }
    }
    ...
    private void showNextSuperToast() {
        ...
        if (superToastPriorityQueue.isEmpty()){
            return;
        }
